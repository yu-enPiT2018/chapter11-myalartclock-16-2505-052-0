...
class SimpleAlarmDialog : DialogFragment(){
    ...
    override fun onAttach(context: Context?) {
        ...
        if(context is SimpleAlarmDialog.OnClickListener) {
            listener = context
        }
    }
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val builder = AlertDialog.Builder(context).apply {
            setMessage("時間になりました！")
            setPositiveButton("起きる") {dialog, which -> listener.onPositiveClick()
            }
            setNegativeButton("あと５分！") {dialog, which -> listener.onNegativeClick()}
        }
